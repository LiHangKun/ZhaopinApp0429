...
public class GangWeiDetailActivity1 extends BaseActivity implements View.OnClickListener {
    ...
    @BindView(R.id.work_detail_salary)
    TextView workDetailSalary;
    @BindView(R.id.work_detail_pos)
    TextView workDetailPos;
    @BindView(R.id.work_detail_jingli)
    TextView workDetailJingli;
    @BindView(R.id.work_detail_xueli)
    TextView workDetailXueli;
    @BindView(R.id.work_detail_warn)
    TextView workDetailWarn;
    @BindView(R.id.line1)
    View line1;
    @BindView(R.id.work_detail_duty_title)
    TextView workDetailDutyTitle;
    @BindView(R.id.work_detail_duty_info)
    TextView workDetailDutyInfo;
    @BindView(R.id.work_detail_inquire_title)
    TextView workDetailInquireTitle;
    @BindView(R.id.work_detail_inquire_info)
    TextView workDetailInquireInfo;
    @BindView(R.id.line2)
    View line2;
    ...
    TextView workDetailCompanyInfoTitle;
    ...
    @BindView(R.id.group)
    Group group;
    @BindView(R.id.work_detail_interview_pos)
    TextView workDetailInterviewPos;
    @BindView(R.id.line5)
    View line5;
    @BindView(R.id.work_detail_interview_pos_title)
    TextView workDetailInterviewPosTitle;
    ...
    protected void initView(Bundle savedInstanceState) {
        setContentView(R.layout.activity_gangweidetail);
        ...
        OkHttpHelper.getInstance().post(mContext, NetClass.BASE_URL + NetCuiMethod.zhiWeiDetail, params, new SpotsCallBack<ZhiWeiDetailBean>(mContext) {
            ...
            public void onSuccess(Response response, ZhiWeiDetailBean resultBean) {
                ...
                switch (positionType) {
                    ...
                    case "1":
                        workDetailWarn.setVisibility(View.GONE);
                ...
                workDetailName.setText(resultBean.getName());
                String salary_format = "%s—%sk";
                String pos_format = "%s%s";
                workDetailSalary.setText(String.format(salary_format, resultBean.getMinSalary(), resultBean.getMaxSalary()));
                workDetailPos.setText(String.format(pos_format, resultBean.getCity().getName(), resultBean.getDistrict().getName()));
                workDetailJingli.setText(resultBean.getExperienceYear().getName());
                workDetailXueli.setText(resultBean.getEducation().getName());
                workDetailInquireInfo.setText(resultBean.getSkills());
                ...
                String company_info_format = "%s | %s | %s";
                workCompanyInfoShort.setText(String.format(company_info_format, resultBean.getCompany().getFinancing().getName(),
                        resultBean.getCompany().getIndustry().getName(),
                        resultBean.getCompany().getStaffNum()));
                workDetailInterviewPos.setText(resultBean.getCompany().getLocation());
    ...
    */
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.image2:
                ...
                if (!TextUtils.isEmpty(SPTool.getSessionValue(AppSP.UID))) {
                    if (!TextUtils.isEmpty(shareTitle)) {
                        ...
                        new ShareUtils(GangWeiDetailActivity1.this).share(shareUrl, AppUtils.getAppName(GangWeiDetailActivity1.this), shareTitle, NetClass.Share_AppLogo);
                    }
                } else {
                    ToastFactory.getToast(mContext, "请先登录").show();
                    startActivity(new Intent(mContext, Login1PhoneCodeActivity.class));
            ...
            case R.id.work_detail_duty_info:
                //岗位详情的文本,查看全部
                if (v.isSelected()) {
                    //如果是收起的状态
                    workDetailDutyInfo.setText(Cui);
                    workDetailDutyInfo.setSelected(false);
                } else {
                    //如果是展开的状态
                    workDetailDutyInfo.setText(Cui);
                    workDetailDutyInfo.setSelected(true);
                }
                break;
        }
    }
